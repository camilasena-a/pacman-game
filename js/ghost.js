class Ghost {
  ...
  checkTeleport() {
      ...
      // Check if the ghost is on the tunnel line (with a small tolerance)
      const ghostRow = Math.floor(this.y / oneBlockSize);
      const isTunnelRow = (ghostRow === tunnelRow || 
                         // Larger tolerance to allow vertical movement after teleportation
                         Math.abs(this.y - (tunnelRow * oneBlockSize)) < oneBlockSize);
      
      if (!isTunnelRow) {
          return; // Do not allow teleportation outside the tunnel line
      }
      
      const rightBorder = map[0].length * oneBlockSize;
      const leftBorder = 0;
      
      // Check if the ghost touched the left border
      if (this.x <= leftBorder && this.direction === DIRECTION_LEFT) {
          // Teleport immediately to the right side
          this.x = rightBorder - this.width;
      }
      // Check if the ghost touched the right border
